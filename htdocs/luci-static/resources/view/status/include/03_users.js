...
function renderTitle(title, users, collapseID) {
  ...
  const collapseBtn = E('button', { style: css.button }, Store.showUsers ? '⏫' : '⏬');
...
function toggleIconsCard() {
  ...
  card.style.transformOrigin = `${Store.iconsCard.oX}px ${Store.iconsCard.oY}px`;
  card.style.transform = `scale(${Store.iconsCard.show ? 1 : 0}) translate(${Store.iconsCard.tX}px, ${Store.iconsCard.tY}px)`;
...
function renderUserIcon(info) {
  ...
  userBtn.addEventListener('click', () => {
    ...
    Store.iconsCard.show = Store.thisUser.id != Store.lastUser.id ? true : !Store.iconsCard.show;
    toggleIconsCard();
  });
...
return baseclass.extend({
  ...
  render: function (data) {
    ...
    return E('div', { style: css.box }, [
      renderIconsCard(data[4]),
      renderTitle(_('WiFi在线用户'), users.length, 'wifi-users'),
      E('div', { id: 'wifi-users', style: css.grid }, users)
    ]);
  }
});